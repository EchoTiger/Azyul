...
namespace Honorbuddy.Quest_Behaviors.EncounterUtility
{
    [CustomBehaviorFileName(@"EncounterUtility")]
    public class EncounterUtil : CustomForcedBehavior
    {
        /// <summary>
        ...
        /// </summary>
        /// 
        public EncounterUtil(Dictionary<string, string> args)
            : base(args)
        {
            try
            {
                ...
                AuraId = GetAttributeAsNullable<int>("AuraId", true, ConstrainAs.MobId, new[] { "NpcId", "NpcID" }) ?? 0;
				BossId = GetAttributeAsNullable<int>("BossId", true, ConstrainAs.MobId, new[] { "NpcId", "NpcID" }) ?? 0;
				AddIds = GetNumberedAttributesAsArray<int>("AddId", 0, ConstrainAs.MobId, new[] { "NpcId" });
				FollowMobIds = GetNumberedAttributesAsArray<int>("FollowMobId", 0, ConstrainAs.MobId, new[] { "NpcId" });
            }
		...
		private int[] FollowMobIds { get; set; }
		...
		private WoWUnit FollowMob
        {
            get
            {   return (ObjectManager.GetObjectsOfType<WoWUnit>().Where(u => FollowMobIds.Contains((int)u.Entry) && !u.IsDead && u.IsFriendly).OrderBy(
		...
		public Composite MobFollow
        {
            get
            {
                return new Decorator(r=> FollowMob != null && Boss.HasAura(AuraId) && (FollowMob.IsMoving || FollowMob.Distance > 9),
				
				new Sequence(
                                    new Action(ret => TreeRoot.StatusText = "Following Mob - " + FollowMob.Name + " At X: " + FollowMob.X + " Y: " + FollowMob.Y + " Z: " + FollowMob.Z),
                                    new Action(ret => Navigator.MoveTo(FollowMob.Location)),
        ...
        public Composite KillAdds
        {
            get
            {
                return new Decorator(r=> Adds != null && Boss.HasAura(AuraId) && (FollowMob == null || (Adds.Distance < 8 && !FollowMob.IsMoving)), new PrioritySelector(
        ...
        {
            return _root ?? (_root = new Decorator(ret => !_isBehaviorDone,new PrioritySelector(DoneYet, MobFollow, KillAdds, KillBoss)));
        }
